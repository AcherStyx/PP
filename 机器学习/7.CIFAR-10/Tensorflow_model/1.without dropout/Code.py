...
with tf.Session() as sess:
    ...
    for i in range(TRAINING_STEPS):
        ...
        if i%CHECK_FREQUNCY==0:
            ...
            print(OUTPUT_FORMAT_STRING.format(step=i,acc=sess.run(accuracy,feed_dict=accuracy_test_dict),lr=sess.run(learning_rate),ce=sess.run(cross_entropy,feed_dict=accuracy_test_dict),re=sess.run(regularization,feed_dict=accuracy_test_dict),accit=sess.run(accuracy,feed_dict=accuracy_test_train_dict),srlt=sess.run(label,feed_dict=accuracy_test_dict)))
